...
import { AppState, Linking, Pressable, ScrollView, StyleSheet, Text } from 'react-native';
    ...
    return (
      <ScrollView>
        ...
        <Pressable
          ...
          style={({ pressed }) => ({ minWidth: 48, minHeight: 48, opacity: pressed ? 0.7 : 1 })}
          onPress={() => {
...
function CameraPreview() {
  ...
  if (!device)
    return (
      <ScrollView>
        <Text style={styles.message}>A back LiDAR video/depth camera is required (#18).</Text>
      </ScrollView>
    );
  if (cameraError)
    return (
      <ScrollView>
        <Text style={styles.message}>Camera unavailable. Reopen Scan to try again (#18).</Text>
      </ScrollView>
    );
