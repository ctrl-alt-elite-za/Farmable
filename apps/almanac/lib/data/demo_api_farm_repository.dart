...
class DemoApiFarmRepository implements FarmRepository {
  ...
  Future<Dashboard> startSession() async {
    final body = await _send('POST', '/demo/sessions', body: const {}, authed: false);
  ...
  Future<PlanningResult> preview(String sectionId, PlanRequest request) async =>
      PlanningResult.fromJson(
        await _send('POST', '/demo/sections/$sectionId/preview', body: request.toJson()),
      );

  @override
  Future<SavedPlan> savePlan(String sectionId, PlanRequest request) async =>
      SavedPlan.fromJson(await _send(
        'POST',
  ...
  @override
  Future<SavedPlan> approvePlan(String planId) async =>
      SavedPlan.fromJson(await _send(
        'POST',
  ...
  String _idempotencyKey() {
    ...
    return List.generate(24, (_) => alphabet[_random.nextInt(alphabet.length)]).join();
  }
    ...
    try {
      response = await _dio.request<dynamic>(
        ...
        options: Options(
          ...
          headers: {
            ...headers,
            if (authed) 'Authorization': 'Bearer $_token',
          },
        ),
      );
  ...
  FarmRepositoryException _translate(int status, dynamic data) {
    ...
    final code = error is Map ? error['code'] as String? ?? 'unknown' : 'unknown';
