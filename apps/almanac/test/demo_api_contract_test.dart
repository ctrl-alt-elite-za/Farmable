...
void main() {
  ...
  group('session and farm', () {
    ...
    test('an unauthenticated call raises SessionExpired, not a raw error',
        () async {
      if (!up) return;
    ...
    test('an unreachable host raises Unreachable, never a Dio type', () async {
      ...
      final offline =
          DemoApiFarmRepository(baseUrl: 'http://127.0.0.1:1', dio: null);
      await expectLater(offline.startSession(), throwsA(isA<Unreachable>()));
    });
  });

  group('planner', () {
    test('a feasible request returns candidate plans with real figures',
        () async {
      if (!up) return;
      final (r, dashboard) = await session();
      ...
      final cabbage = plan.allocations.firstWhere((a) => a.crop == Crop.cabbage);
      ...
      final summed = estimate.costs
          .fold(0, (total, line) => total + line.cost.value);
      expect(summed, estimate.totalCost.value);
    });

    test('a plan that cannot afford the whole section leaves blocks unplanted',
        () async {
      if (!up) return;
      final (r, dashboard) = await session();
      final section = dashboard.sections.firstWhere((s) => s.isAvailable);
      ...
      expect(
        plan.unplantedAreaM2.asDouble,
        plan.unplantedBlocks * 100.0,
      );
    });

    test('an unaffordable request explains itself instead of returning nothing',
        () async {
      if (!up) return;
      final (r, dashboard) = await session();
      final section = dashboard.sections.firstWhere((s) => s.isAvailable);
  ...
  group('sections', () {
    test('a created section reports a farmer-supplied area', () async {
      ...
      final created = await r.createSection(name: 'North Plot', areaM2: '250.00');
    ...
    test('a stale revision is refused rather than clobbering', () async {
      if (!up) return;
      final (r, _) = await session();
      final created = await r.createSection(name: 'Test Plot', areaM2: '120.00');
