...
5. Keep local files/records when any request fails. This API never instructs local
   deletion. A `failed` upload is terminal; corrected content needs new local media
   and mutation IDs. An expired, never-processed reservation can be reopened by an
