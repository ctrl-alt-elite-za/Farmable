...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "user_id", "consent_type", "version", name="uq_account_consents_version"
        ),
        sa.CheckConstraint(
            "length(trim(consent_type)) > 0",
            name="ck_account_consents_consent_type_nonblank",
        ),
        sa.CheckConstraint(
            "length(consent_type) <= 80", name="ck_account_consents_consent_type_max_length"
        ),
